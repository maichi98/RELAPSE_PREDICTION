...
def get_list_thresholds(imaging, label, feature):
    ...
    dir_list_thresholds = constants.dir_results / 'list_thresholds'
    dir_list_thresholds.mkdir(parents=True, exist_ok=True)

    with open(dir_list_thresholds / f"{label}_{imaging}_{feature}_list_thresholds.pickle", "wb") as f:
...
def print_cutoff(label, imaging, feature, dict_fpr_tpr):
    ...
    with open(f"/media/maichi/SSD-IGR/total_cutoffs.txt", "a+") as f:
        f.write(txt + "\n")
...
def main():
    ...
    for imaging in list_imaging[8: 10]:
